...
> ⚠️ **주의**: 아래 결과는 **G1 채택 이전(~2026-06-10)** 구 파라미터 기반입니다.  
> 현재 전략(G1)과 차이: SL `-7%→-6%`, TP `avg_win 동적→+15% 고정`, 신호 기준 `분봉 장중→EOD 완성 일봉`.  
> G1 실제 성과는 `backtest_walkforward.py` walk-forward 결과 **세후 EV +1.468%** 를 기준으로 합니다.
