...
- 진입 조건 (5중 필터):
  ...
  5. **기대값 손익비 ≥ 1.5** — 약세장(KOSPI < MA20 OR RSI < 35)에서는 `avg_win × 0.4` 적용, ADR < 0.5이면 reversion 에이전트에 `avg_win × 0.25` 추가 엄격화
...
| 미국 | S&P 500 전체 (503종목) | 등락률 > 0% + 거래량 비율 × 1.5 이상 (전체 503종목 스캔) |
...
| 기대값 손익비 | ≥ 1.5 | `(avg_win_eff × win_prob) / (avg_loss × (1−win_prob))` — 약세장(MA/RSI) 시 × 0.4, ADR < 0.5 시 reversion × 0.25 |
...
| 08:00 (영업일) | 모닝 브리핑 — yfinance로 S&P 500·나스닥·다우존스 실제 지수 직접 조회 후 프롬프트 주입 (AI hallucination 방지) + Tavily 뉴스/시황 + LangGraph 품질 재시도 루프 |
...
| **05:20 / 06:20 (평일)** | **US EOD 신호 스캔** — 미국 장 마감 20분 후, S&P 500 전체(503종목) EOD 일봉으로 신호 탐지 → 당일 밤 시초가 예약 (서머/동절기 자동 분기) |
...
└──────┬───────┴─────────────┬─────────────┴───────────────────────────┘
       ...
       │     market_regime.py → KOSPI MA5/MA20/RSI14 + ADR 시장 상황 판정
       │       ├── 역배열(MA5 < MA20): 신규 매수 전면 차단 (보유 청산만 유지)
       │       ├── 약세장(Close < MA20 OR RSI < 35): avg_win × 0.4 패널티 적용
       │       └── ADR < 0.5 (상승종목/하락종목): reversion avg_win × 0.25 추가 패널티
