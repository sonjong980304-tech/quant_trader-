...
- 재학습: **연 1회**(매년 1/1 또는 직후 첫 영업일, `runner.py::retrain_kospi200_annual`,
  `config.KOSPI200_RETRAIN_SCHEDULE`) — 리밸런싱(40거래일마다 매매)과는 독립된 캘린더다.
  (2026-07-22 저녁: 처음엔 40거래일 리밸런싱 시점마다 재학습했으나, 백테스트로 재현해보니
  너무 잦은 재학습이 gross 기준으로도 성과를 깎아먹는 것으로 확인돼 연 1회로 되돌렸다 —
  "KOSPI200 XGB 랭킹 — 국면별·재학습사이클별 성과 분해" 절의 (D) 참조.) walk-forward fold는
  "오늘" 기준 상대 기간으로 매번 새로 계산되고(`ml/kospi200_trainer.py::retrain_kospi200_global`),
  새 모델을 기존 모델과 **동일한 검증구간에 재채점**해서 IC가 더 낮으면 자동 롤백해 기존 모델을
  유지한다(reversion의 AUC/avg_win 롤백 게이트와 같은 취지 — 2026-07-22: "새 모델 IC vs 기존의
  옛날 IC"를 비교하던 결함을 고쳐 동일 기간 비교로 바꿈, 위 (B) 참조).
...
### (B) 재학습 사이클(40거래일마다) 시뮬레이션 — IC 롤백 결함 발견 및 수정

`ml/kospi200_trainer.py`의 원래 로직(직전 배포 모델의 "그때 저장된" IC보다 새 모델 IC가
낮으면 롤백)을 과거 67개 리밸런싱 시점에 그대로 재현했다.

- **1차 재현 결과**: 67개 유효 사이클 중 62개(92.5%)가 롤백됐다. 2019-12-09에 채택된 모델
  (IC=+0.0508)이 2026-06까지 6년 넘게 사실상 그대로 유지됐다 — "40거래일마다 최신 데이터로
  재학습한다"는 설계 의도와 달리, 실제로는 초반에 우연히 IC가 높게 나온 모델에 장기간 고정되는
  패턴이 나타났다.
- **원인 정밀 진단**: 2019-12 챔피언 모델을 고정한 채로 이후 40개 사이클의 검증구간에 매번
  "동일 기간" 재채점을 해봤다. 그 결과 **챔피언이 같은 기간에서 신규 후보보다 실제로 나았던
  경우는 7.5%(3/40)뿐**이었고(동일구간 IC 평균: 챔피언 0.0125 vs 신규후보 0.0219),
  나머지 92.5%는 **더 나은 모델을 계속 만들어놓고도 결함 있는 비교 방식 때문에 버리고 있었다.**
  원래 로직은 "새 모델의 이번 검증연도 IC"와 "챔피언의 그때 검증연도 IC"를 서로 다른 시기
  기준으로 비교했던 게 문제였다 — 2019-12-09가 유난히 우호적인 검증구간(그해 하락장에서도
  IC가 튄 시점)이었을 뿐, 그 모델 자체를 이후 구간에 재채점하면 평범한 수준(평균 0.0125)이라
  감쇠 추세도 없었다(선정 자체는 그 시점 기준 미래 데이터를 안 쓴 정당한 워크포워드였고,
  사후 컷오프가 아니었다 — 문제는 선정 이후의 비교 메커니즘이었다). 신규후보 IC와 그 해
  코스피 수익률의 상관계수는 -0.224로 약해, 시장 국면만으로 설명되는 문제도 아니었다.
- **⚠️ 이전에 이 절에 적었던 "롤백이 성과를 지켰을 수도 있다"는 해석은 정정한다.** 그 해석은
  표본 5개짜리 코호트 실현수익 비교에 근거했는데, 이번처럼 표본 40개로 IC(예측력)를 직접
  비교하니 정반대였다 — 실현수익 비교는 노이즈가 커서 신뢰할 수 없었다.
- 사이클 내 신선(1~20일차) vs 열화임박(21~40일차) 구간 성과 차이(샤프 +0.198 vs +0.239)는
  애초에 챔피언이 거의 안 바뀌는 상태에서 나온 수치라 재해석하지 않는다.
- **프로덕션 수정 완료(2026-07-22)**: `ml/model.py::train_global_regression()`이 이제
  마지막 fold의 검증셋을 함께 반환하고, `ml/kospi200_trainer.py::retrain_kospi200_global()`이
  기존 모델을 그 "동일 검증구간"에 재채점해 새 모델과 직접 비교하도록 고쳤다(`old_ic_same_period`
  vs `ic`). 기존 모델의 학습 당시 저장된 stale IC(`old_ic_stale`)는 로그 참고용으로만 남긴다.
  프로덕션은 최초 학습 1회(IC 0.12~0.14)만 거친 상태라 아직 실제 롤백 이력은 없고, 다음 재학습
  때부터 새 로직이 적용된다(다음 재학습 시점은 아래 (D)에 따라 매년 1/1로 변경됨).
...
### (D) 재학습 주기 비교 — "40거래일마다"에서 "1년에 1번"으로 되돌림 (2026-07-22)

(B)의 롤백 결함을 고친 뒤, 그 수정된 로직을 "40거래일마다 재학습"이라는 원래 설계 그대로
과거에 다시 적용해봤다. 그런데 롤백을 고쳤어도 **애초의 그리드서치 채택안(1년에 1번 재학습)
보다는 여전히 못 미쳤다** — 재학습 주기 자체를 재검토하게 된 계기다.

| 재학습 주기 | 넷 샤프 | 넷 소르티노 | 넷 총수익 | 평균IC | ICIR |
|---|---:|---:|---:|---:|---:|
| **1년에 1번(채택, 12폴드)** | **0.480** | **0.666** | **+159.98%** | 0.0167 | 0.788 |
| 40일마다(롤백 결함, 67사이클) | 0.219 | 0.295 | +28.09% | 0.0220 | 1.565 |
| 40일마다(롤백 수정, 67사이클) | 0.317 | 0.437 | +63.22% | 0.0253 | 1.722 |

(HOLD=REBAL_FREQ=40 리밸런싱 주기는 세 경우 모두 동일 — 오직 "모델을 얼마나 자주 다시
학습하는지"만 다르다. 40일 쪽 ICIR은 67개 개별 사이클 기준이며, 1년 쪽과 같은 표본크기(12개
연도버킷)로 다시 묶어 계산해도 1.555로 큰 차이가 없었다 — 표본수 문제가 아니라 실질적 차이다.)

**예측력(IC/ICIR)은 40일마다 재학습하는 쪽이 오히려 더 높고 꾸준한데, 실현 성과(샤프)는 거꾸로
1년에 1번이 확실히 앞선다.** "매매비용(회전율) 때문 아니냐"는 가설을 gross(비용 차감 전) 수익률로
직접 검증했다:

| | gross 샤프 | net 샤프 |
|---|---:|---:|
| 1년에 1번 | 0.601 | 0.480 |
| 40일마다(수정 로직) | 0.444 | 0.317 |
| 격차(1년 − 40일) | +0.158 | +0.162 |

비용을 떼기 전 단계에서 이미 격차(+0.158)가 나 있고 비용을 뗀 뒤에도 거의 그대로(+0.162)라,
**회전율/비용 가설은 기각됐다.** IC가 좋아도 실현 성과가 나쁜 이유는 아직 확정하지 못했지만
(가설: 모델을 자주 바꾸면 종목 선정의 방향성이 리밸런싱마다 끊겨서, 한 모델을 오래 쓸 때
생기는 "같은 팩터 사이클을 여러 리밸런싱에 걸쳐 타는" 효과가 사라지는 것으로 추정 — 확정된
결론은 아님), **재학습 주기는 1년에 1번으로 되돌렸다**(`config.KOSPI200_RETRAIN_SCHEDULE`,
`runner.py::retrain_kospi200_annual()` — 매년 1/1 또는 직후 첫 영업일 07:31 실행, 리밸런싱
스캔 `scan_kospi200_signals_eod()`와 독립된 캘린더). 리밸런싱(40거래일마다 매매) 주기는
그리드서치 채택안 그대로 유지한다 — 이건 재학습 주기와는 별개 파라미터다.
...
| 시간 | 동작 |
|------|------|
| 07:30 (1/4/7/10월 1일 또는 직후 첫 영업일) | **분기별 ML 모델 재학습** — reversion XGBoost(Expanding Window + PIT 시총 상위 200). trend는 규칙 기반이라 재학습 없음 |
| **07:31 (매년 1/1 또는 직후 첫 영업일)** | **kospi200_xgb 연간 ML 모델 재학습** — 기존 모델과 동일 검증구간 재채점 비교, IC 낮으면 롤백(2026-07-22: 40거래일마다→연 1회로 되돌림, 아래 (D) 참조) |
...
| **15:32 (영업일)** | **KOSPI200 XGB 랭킹 스캔** — kospi200_xgb 보유 포지션 0개일 때만(코호트 청산 완료 후) 그 시점 배포 모델로 상위 10종목 스캔(재학습은 하지 않음 — 07:31에 연 1회만) |
