...
- 한국 주식: **KIS API 실시간 현재가** / 미국 주식: **yfinance 5분봉** 장중 신호 감지
- 신호 즉시 **자동 매수** (켈리 0주이면 스킵, 확인 없음)
- 진입 조건 (4중 필터):
  1. 기술적 트리거 **1개 이상** 충족
  2. 모델 **AUC ≥ 0.58** (동전던지기 수준 모델 차단)
  3. **승률 ≥ 60%** (Platt Scaling 캘리브레이션 적용)
  4. **기대값 손익비 ≥ 1.5** (`avg_win × 승률 / avg_loss × 패률`)
- 포지션 사이징: **켈리 공식** (최대 25% 한도)
- 07:30 **KRX 상위 100 유니버스 재학습** / 22:30·23:30 **US 상위 100 유니버스 재학습** (서머/동절기 자동 분기)
...
> 5가지 중 **1개 이상** 충족 시 ML 예측 단계로 진입. 주 품질 필터는 AUC ≥ 0.70 + 승률 ≥ 65%  
> 한국 주식: KIS API 실시간 바 합성 / 미국 주식: yfinance 5분봉으로 오늘 바 실시간 합성
...
| 기대값 손익비 | ≥ 1.5 | `(avg_win × win_prob) / (avg_loss × (1−win_prob))` |
...
└──────┬───────┴─────────────┬─────────────┴───────────────────────────┘
       │                     │
       │    ┌────────────────▼──────────────────────────────────────┐
       │    │         langchain_agent.py  (LangChain + gpt-5.5)      │
       │    │   ConversationBufferWindowMemory(k=10) 유저별 유지      │
       │    │   10종 툴 자동 호출 + Context Recall 평가 (gpt-5.4-mini)│
       │    └───────────────────────────────────────────────────────┘
       │
       ├── [07:30] ml/trainer.py — KRX 100+US 100 유니버스 XGBoost 병렬 재학습
       │
       ...
       │     signals/krx_universe.py → FinanceDataReader KOSPI+KOSDAQ 스크리닝 (100개)
       │     signals/us_universe.py  → S&P 500 전체 1차 스크리닝 (50개)
       │       ↓ KIS API 실시간 바(한국) / yfinance 5분봉(미국) + 오늘 바 합성
       │     signals/scanner.py → 기술적 트리거 5종 중 1개 이상 감지
       │       ↓ XGBoost 예측 + Platt Scaling (AUC≥0.58 AND 승률≥60% AND 기대값 손익비≥1.5)
       │     → 텔레그램 알림 + 켈리 공식 자동 매수 (0주이면 스킵)
       │
