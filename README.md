...
- 진입 조건 (4중 필터):
  1. 기술적 트리거 **2개 이상** 동시 충족
  2. 모델 **AUC ≥ 0.70** (동전던지기 수준 모델 차단)
  3. **승률 ≥ 65%**
...
> **5가지 중 2개 이상 동시 충족**해야 ML 예측 단계로 진입 (1개 단독 트리거는 노이즈로 제외)  
...
| 필터 | 기준 | 설명 |
|------|------|------|
| 모델 AUC | ≥ 0.70 | OOF AUC 0.5 = 동전던지기, 0.7 이상만 신뢰 |
| 승률 | ≥ 65% | 현재 바 피처 기반 7일 내 +3% 달성 확률 |
| 기대값 손익비 | ≥ 1.5 | `(avg_win × win_prob) / (avg_loss × (1−win_prob))` |
| 트리거 수 | 2개 이상 | 단독 트리거는 노이즈로 차단 |
...
└──────┬───────┴─────────────┬─────────────┴───────────────────────────┘
       ...
       │     signals/scanner.py → 기술적 트리거 5종 중 2개 이상 감지
       │       ↓ XGBoost 예측 (AUC≥0.70 AND 승률≥65% AND 기대값 손익비≥1.5)
