...
넷 샤프(+0.480)와 함께 넷 소르티노(하방 변동성만으로 계산한 위험조정수익, +0.666)도 확인했다.
같은 기준 KOSPI buy&hold의 소르티노는 +1.004로 더 높다 — 위와 동일하게 2025하반기 폭등장 쏠림
때문이며, 아래 국면별 분해에서 그 구간을 뺀 비교를 별도로 제공한다.

---

## KOSPI200 XGB 랭킹 — 국면별·재학습사이클별 성과 분해 (`quant_trader_backtest_dev`, 2026-07-22)

그리드서치로 채택된 HOLD=REBAL=40 설정을 두 축으로 더 쪼개 분석했다: (A) 시장 국면별로 강점이
어디 있는지, (B) 프로덕션과 동일한 "40거래일마다 재학습 + IC 낮으면 롤백" 로직이 과거에는 실제로
어떻게 동작했을지. 연도별 표는 참고용 보조 지표로 맨 아래에 둔다.

### (A) 국면(레짐)별 성과

KOSPI 6개월(126거래일) 수익률로 상승장(+10%↑)/횡보장/하락장(-10%↓) 라벨링 후, 국면별 net 전략
vs KOSPI buy&hold 비교(일수는 전체 2015~2026 거래일 기준).

| 국면 | 비중 | 전략net 누적수익률 | 전략net 연율화수익률 | 전략 샤프 | 전략 소르티노 | KOSPI 누적수익률 | KOSPI 연율화수익률 | KOSPI 샤프 | KOSPI 소르티노 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 상승장 | 26.1% | +101.04% | +27.42% | 1.053 | 1.393 | +368.96% | +54.03% | 1.875 | 2.062 |
| 횡보장 | 62.7% | +79.54% | +10.41% | 0.515 | 0.776 | +35.89% | +4.22% | 0.280 | 0.379 |
| 하락장 | 11.2% | -27.97% | -20.41% | -0.594 | -0.844 | -52.91% | -43.11% | -1.561 | -2.152 |

(누적수익률은 그 국면에 해당하는 날짜들의 수익률만 순서대로 이어붙여 복리 계산한 값 — 국면이
여러 구간에 흩어져 있어도 "그 국면에만 투자했다면"의 총수익을 보여준다.)

**전체기간의 63%를 차지하는 횡보장에서 전략이 코스피를 샤프·소르티노 모두 확실히 앞선다**
(누적수익률도 +79.54% vs +35.89%로 2배 이상). 상승장(코스피가 몰아서 급등하는 구간)은 코스피가
워낙 압도적으로 뛰어(+368.96%) 전략이 못 따라가고, 하락장에서는 둘 다 손실이지만 전략의 손실
폭이 코스피의 절반 수준이다(-27.97% vs -52.91%, 샤프 -0.594 vs -1.561). 정리하면 이 전략은
**평상시엔 코스피 대비 확실한 우위, 극단적 상승장엔 열세, 하락장엔 상대적으로 방어적**이라는
특성을 국면 단위로도 재확인했다 — 그리드서치 절 "2025하반기 절단 비교"와 같은 결론이나 이번엔
연도가 아니라 시장 상황 자체로 쪼갠 결과라는 점이 다르다.

### (B) 재학습 사이클(40거래일마다) 시뮬레이션 — IC 롤백 실제 동작 재현

`ml/kospi200_trainer.py`와 동일한 로직(직전 배포 모델의 IC보다 새 모델 IC가 낮으면 롤백)을
과거 67개 리밸런싱 시점에 그대로 재현했다.

- **67개 유효 사이클 중 62개(92.5%)가 롤백됐다.** 2019-12-09에 채택된 모델(IC=+0.0508)이
  2026-06까지 6년 넘게 사실상 그대로 유지됐다 — "40거래일마다 최신 데이터로 재학습한다"는
  설계 의도와 달리, 실제로는 초반에 우연히 IC가 높게 나온 모델에 장기간 고정되는 경향이
  관찰됐다.
- **원인 추정**: 연도별 IC 자체의 편차가 크다(관측범위 -0.012~+0.051). 현재 로직은 "새 모델의
  이번 검증연도 IC"와 "챔피언 모델의 그때 검증연도 IC"를 직접 비교하는데, 서로 다른 해를
  비교하는 셈이라 모델 품질보다 "그 해가 이 전략에 얼마나 우호적이었는지"에 좌우될 수 있다.
- **다만 부정적 결과만은 아니다**: 롤백된 사이클의 평균 코호트 net수익(+1.39%, n=61)이 신규
  채택된 사이클(-5.03%, n=5)보다 높았다 — 표본이 5개뿐이라 확정할 순 없지만, 결과적으로
  "자주 롤백된 것"이 성과를 해치지 않았을 가능성도 있다.
- 사이클 내 신선(1~20일차) vs 열화임박(21~40일차) 구간 성과 차이는 유의미하지 않았다
  (샤프 +0.198 vs +0.239) — 챔피언이 거의 안 바뀌다 보니 애초에 "사이클 간 신선도 차이"가
  실질적으로 크지 않았기 때문으로 보인다.
- **프로덕션 반영 여부**: 로직은 현재 그대로 유지한다(2026-07-22 결정). 프로덕션은 최초 학습
  1회(IC 0.12~0.14, 위 시뮬레이션과는 학습기간·피처분포가 달라 수치를 직접 비교할 수는 없음)만
  거친 상태라 아직 실제 롤백 이력이 없다. 이 "장기 고정" 패턴이 실제로 재현되는지는 향후 재학습
  로그(`ml/kospi200_trainer.py` 실행 로그의 `rolled_back`/`old_ic`/`ic`)로 계속 관찰한다.

### (C) 연도별 수익률·샤프·소르티노 (보조 요약)

| 연도 | 전략net% | 전략샤프 | 전략소르티노 | KOSPI% | KOSPI샤프 | KOSPI소르티노 |
|---|---:|---:|---:|---:|---:|---:|
| 2015 | +18.16 | 1.078 | 1.847 | +2.38 | 0.207 | 0.305 |
| 2016 | -2.46 | -0.172 | -0.249 | +5.61 | 0.335 | 0.417 |
| 2017 | +14.41 | 0.921 | 1.415 | +21.78 | 2.242 | 3.263 |
| 2018 | +11.16 | 0.711 | 0.939 | -17.69 | -1.292 | -1.675 |
| 2019 | +0.98 | 0.011 | 0.018 | +9.34 | 0.666 | 0.878 |
| 2020 | +36.23 | 1.081 | 1.447 | +32.10 | 1.105 | 1.385 |
| 2021 | +12.31 | 0.710 | 1.087 | +1.13 | 0.302 | 0.495 |
| 2022 | -12.92 | -0.430 | -0.638 | -25.17 | -1.482 | -2.288 |
| 2023 | -6.10 | -0.236 | -0.376 | +19.30 | 1.224 | 2.022 |
| 2024 | -9.81 | -0.371 | -0.529 | -10.13 | -0.434 | -0.536 |
| 2025 | +51.51 | 1.783 | 2.760 | +75.67 | 2.834 | 3.664 |
| 2026 | +8.94 | 0.651 | 0.892 | +110.05 | 3.405 | 4.154 |

2019·2022~2024년처럼 전략이 코스피와 방향이 다르거나 부진한 해가 섞여 있다 — 국면별 분해(A)가
보여주듯 이는 그 해의 코스피 국면(급등/급락/횡보) 구성에 좌우되는 부분이 크다.
