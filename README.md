...
- 신호 즉시 **자동 매수** (켈리 0주이면 주문가능금액으로 재시도, 1주도 불가 시 스킵)
- 진입 조건 (5중 필터):
...
└──────┬───────┴─────────────┬─────────────┴───────────────────────────┘
       ...
       │     signals/us_universe.py  → S&P 500 전체 1차 스크리닝 (회사명 포함, 100개)
       │       ↓ KIS API 실시간 바(한국) / yfinance 5분봉(미국) + 오늘 바 합성
       │     signals/scanner.py → 기술적 트리거 5종 중 1개 이상 감지 + ATR(14) 계산
       │       ↓ 트리거 유형별 에이전트 라우팅
       ...
       │     → 텔레그램 알림 (ATR손절가 표시) + min(켈리, 리스크동등화) 수량 자동 매수
       │
...
├── paper_trader.py         # 페이퍼 트레이딩 엔진 (KR+US, Circuit Breaker, P4 게이트, 일일 리포트)
├── tests/
│   ├── test_triple_barrier.py  # Triple-Barrier 라벨링 단위 테스트
│   └── test_paper_trader.py    # 페이퍼 트레이딩 엔진 단위 테스트 (58케이스)
...
| 기능 | 설명 |
|------|------|
| P1 신호 기록 | `log_paper_signal()` — 신호 발생 시 종목·체결가·슬리피지 기록 |
