...
def _search(query: str, k: int = 5) -> str:
    ...
    results = client.search(query, max_results=k)["results"]
    return "\n\n".join(
        f"[출처: {r['url']}]\n{r['content']}"
        for r in results
    )
...
def _gather_context() -> dict:
    ...
    queries = {
        "us_market":     f"{today} US stock market overnight close S&P500 Nasdaq Dow recap",
        "stock_news":    f"{today} ({stock_list}) 주식 뉴스 오늘",
        "econ_calendar": f"{today} economic calendar events CPI FOMC Fed rate decision schedule",
    }
...
1. 간밤 미국 증시 마감시황을 요약해줘. 상승/하락의 원인을 한 가지만 짚어줘.
