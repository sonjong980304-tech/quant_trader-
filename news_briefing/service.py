...
def _is_published_today(published_at: str, now: datetime) -> bool:
    """published_at(네이버 API RFC822 문자열)이 now와 같은 날(KST)인지 판정. 파싱 실패 시 제외."""
    try:
        pub = parsedate_to_datetime(published_at)
    except (TypeError, ValueError):
        return False
    if pub.tzinfo is not None:
        pub = pub.astimezone(_KST).replace(tzinfo=None)
    return pub.date() == now.date()
...
def run_evening(now_fn=None, market_data_fn=None, sleep_fn=None, llm_call=None, send_fn=None,
    ...
    # 당일(KST) 발행 기사만 유지 — NAVER_QUERY_SORT="sim"(유사도순)이라 며칠 지난
    # 기사도 섞여 들어오므로, LLM 선별 전에 미리 걸러 정확도와 토큰 비용을 함께 개선.
    _before = len(naver_articles)
    _now = now_fn()
    naver_articles = [
        a for a in naver_articles if _is_published_today(a.get("published_at", ""), _now)
    ]
    logger.info("[Service] 당일 기사 필터: %d건 → %d건", _before, len(naver_articles))
    ...
    # cause(단순 급등락 원인 설명)는 새 정보가 아니라 사후 해설에 가까워 제외.
    # event(실적·계약 등)·risk(소송·규제 등 경보)는 유지.
    stock_news = [sn for sn in stock_news if sn.get("category") != "cause"]
