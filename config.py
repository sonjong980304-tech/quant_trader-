...
# 2026-07-22 저녁: 재학습 주기를 "40거래일마다"에서 "1년에 1번"으로 되돌림.
# quant_trader_backtest_dev 백테스트로 과거 사이클을 재현해보니 40일마다 재학습이 1년에
# 1번보다 gross(비용 차감 전) 기준으로도 뚜렷이 못했다(넷 샤프 0.480 vs 0.317, gross도
# 0.601 vs 0.444 — 회전율/비용 문제가 아니라 잦은 재학습 자체가 모델 안정성을 해치는 것으로
# 확인, README "재학습 주기 비교" 절 참조). 리밸런싱(매매, 위 KOSPI200_HOLD/REBAL_FREQ=40)은
# 그리드서치로 최적화된 채택안 그대로라 손대지 않는다 — 재학습 주기만 별도 캘린더로 분리한다.
KOSPI200_RETRAIN_SCHEDULE = ['01-01']  # 연 1회 재학습(그리드서치 검증 당시와 동일한 캘린더 경계)
