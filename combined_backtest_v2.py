...
print(f"  학습방식 변경 효과 (A→B):       {r_b_ret - r_a_ret:+.2f}%p  (Rolling이 {'유리' if r_b_ret > r_a_ret else '불리'})")
print(f"  생존편향 제거 효과 (B→C):       {r_c_ret - r_b_ret:+.2f}%p  ({'생존편향 영향 확인됨' if r_c_ret < r_b_ret else '편향 미미'})")
print(f"  전체 변경 효과 (A→C):           {r_c_ret - r_a_ret:+.2f}%p")
